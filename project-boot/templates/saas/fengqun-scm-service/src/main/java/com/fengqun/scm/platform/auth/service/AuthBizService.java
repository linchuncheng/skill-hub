package com.fengqun.scm.platform.auth.service;

import com.fengqun.scm.common.config.JwtProperties;
import com.fengqun.scm.common.exception.BusinessCode;
import com.fengqun.scm.common.exception.BusinessException;
import com.fengqun.scm.common.security.TokenKit;
import com.fengqun.scm.platform.auth.dto.LoginReqDTO;
import com.fengqun.scm.platform.auth.dto.LoginRespDTO;
import com.fengqun.scm.platform.auth.dto.MenuDTO;
import com.fengqun.scm.platform.auth.dto.UserInfoDTO;
...
import com.fengqun.scm.platform.mapper.SysLoginLogMapper;
import com.fengqun.scm.platform.mapper.SysMenuMapper;
import com.fengqun.scm.platform.mapper.SysPermissionMapper;
import com.fengqun.scm.platform.mapper.SysUserMapper;
...
public class AuthBizService {
    ...
    /**
     * 构建菜单树
     * 自动补全缺失的父菜单，确保子菜单不会因为缺少父节点而丢失
     */
    private List<MenuDTO> buildMenuTree(List<SysMenu> menus) {
        // 自动补全缺失的父菜单：遍历所有菜单，如果其 parentId 不在当前集合中，
        // 则从全量菜单中查找并补充，避免子菜单因缺少父节点而被静默丢弃
        List<SysMenu> allMenus = sysMenuMapper.selectAllEnabledMenus();
        Map<Long, SysMenu> allMenuMap = allMenus.stream()
        ...
        List<SysMenu> enrichedMenus = new ArrayList<>(menus);

        for (SysMenu menu : menus) {
            Long parentId = menu.getParentId();
            // 向上追溯，补充所有缺失的祖先菜单
            while (parentId != null && parentId != 0L && !existingIds.contains(parentId)) {
                SysMenu parent = allMenuMap.get(parentId);
                if (parent == null) {
                    break;
                }
                enrichedMenus.add(parent);
        ...
        // 按 parent_id 分组
        Map<Long, List<SysMenu>> menuMap = enrichedMenus.stream()
