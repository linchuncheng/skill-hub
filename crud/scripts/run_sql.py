...
def load_config(config_file=None, env_name=None):
    ...
    if config_file:
        ...
        config_path = os.getenv('AGENT_MYSQL_CONFIG', 
                               os.path.expanduser('~/.config/crud/config.json'))
...
def execute_transaction(sql_statements, config, max_rows=1000):
    """执行事务（多条 SQL）"""
    connection = None
    try:
...
def execute_sql(sql, config, max_rows=1000):
    """执行 SQL 并返回结果"""
    connection = None
    try:
        connection = pymysql.connect(**config)
            ...
            if is_select:
                ...
                if results:
                    ...
                    for header in headers:
                        ...
                        col_widths[header] = min(col_widths[header], 50)  # 最大50字符
...
def execute_sql_file(file_path, config, max_rows=1000):
    """执行 SQL 文件"""
    ...
    for i, sql in enumerate(statements, 1):
        ...
        result = execute_sql(sql, config, max_rows)
...
示例:
  ...
  %(prog)s --init  # 交互式配置数据库连接
    ...
    # 仅初始化配置
    if args.init:
        config_path = os.path.expanduser(args.config if args.config else '~/.config/crud/config.json')
        create_config(config_path)
        return
    ...
    # 执行 SQL
    if args.file:
        results = execute_sql_file(args.file, config, args.max_rows)
        if args.json:
            print(json.dumps(results, indent=2, ensure_ascii=False, default=str))
    elif args.transaction:
        # 事务模式
        if len(args.sql) < 2:
            print("错误: 事务模式至少需要 2 条 SQL 语句")
            sys.exit(1)
        result = execute_transaction(args.sql, config, args.max_rows)
        if args.json:
            print(json.dumps(result, indent=2, ensure_ascii=False, default=str))
    else:
        # 单条 SQL 模式
        if len(args.sql) == 1:
            result = execute_sql(args.sql[0], config, args.max_rows)
            if args.json:
                print(json.dumps(result, indent=2, ensure_ascii=False, default=str))
        else:
            # 多条 SQL 但不使用事务（逐条执行）
            ...
            for i, sql in enumerate(args.sql, 1):
                ...
                print(f"{'='*60}")
                result = execute_sql(sql, config, args.max_rows)
                results.append(result)
